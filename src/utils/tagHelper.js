...
exports.hasExistingTag = async (store, orderId) => {
  ...
  return { hasOurTag, shopifyTags, storeTags, existingTagsString };
};

exports.findAndApplyTag = async (store, orderId, meaning, channel, existingTagsString = null) => {
  const tag = await Tag.findOne({
    where: { store_id: store.id, meaning, channel },
  });
  if (!tag) {
  ...
  const result = await addTagToOrder(store, orderId, tag.name, existingTagsString);
  if (!result.success) {
